...
1. After turning the VM instance on in Google CLoud, go to PoweShell and open "Run as Administrator" then do ssh de-zoomcamp to be able to login
to VM instance (Might need to do step 10 also)

2. After that, we will turn on the docker image by firstly doing cd bin/ and then ./docker-compose

3. Do source .bashrc and then head to data engineering directory where we have docker-compose yaml file

4. Do "docker-compose up -d" to turn on the images, do "docker ps" to see images running from root directory

5. pgcli is already installed and it can be accessed in VM instance using "pgcli -h 127.0.0.1 -p 5431 -U root -d ny_taxi"

6. Make sure that VS Code is open and the remote SSH is active to our Google VM instance and the port is pointing to 5431

7. To access pgcli from the local machine do "pgcli -h 127.0.0.1 -p 5431 -U root -d ny_taxi" again in PowerShell by opening in "Run as Administrator" mode

8. The password for root is root in pgcli on both local and VM instance

9. Add port 5431, 8080 and 8888 in VS Code and it shall show PgAdmin 4 on localhost:8080 and 8888 is for JuPyter notebook

10. When reconnecting the VM, first turn on the VM instance in GCP then come to Git Bash and do "nano .ssh/config" to change the HostName which 
is the external IP of the VM instance, change it iff it changes in the VM otherwise just do "ssh de-zoomcamp" to enter the VM instance

11. Everytime when GCP VM is turned on, we have to follow some steps to activate terraform. We have already put the .json key file in ~/.gc/ in VM instance

12. After that, go into terraform directory "~/data-engineering-zoomcamp/week_1_basics_n_setup/1_terraform_gcp/terraform" and do "export GOOGLE_APPLICATION_CREDENTIALS=~/.gc/igneous-ethos-375619.json"

13. Then do "gcloud auth activate-service-account --key-file $GOOGLE_APPLICATION_CREDENTIALS" so that your GCP service account is activated

14. Then do "terraform init", "terraform plan" and "terraform apply" to make a BigQuery schema and a Cloud Storage bucket. Just remember the GCP Project ID: igneous-ethos-375619